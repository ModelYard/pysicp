...
\chapterQuote{%
    ...
    about what the symbols stand for. \( \dots \)[The mathematician] need not
...
\noindent
\lettrine{W}{e concentrated in} \link{Chapter 1} on computational processes and on the role
...
parameters, and how to abstract procedures by using \code{def}.  We saw that
...
This is the chapter where the two languages part company in earnest.
\link{Chapter 1} could be translated almost line for line: a procedure that
computes with numbers looks much the same in either language, and what differed
was mostly notation.  Data is another matter.  We shall therefore go over the
ground twice---first following the original closely, so that the argument it is
making stays visible, and then again in the way a Python programmer would
actually write it.  The second pass is not a correction of the first.  The
Pythonic version is usually the original's version with the machinery hidden,
and a reader who has seen the machinery knows what the convenience is standing
on.

Two differences will need watching.  The first is which way the language leans
on mutability.  Both languages can change data in place and both can decline
to; what differs is the default and the habit.  The original never changes a
piece of data once it is made---its operations build new structures out of old
ones, and modification in place is held back until \link{Chapter 3}, where it
arrives as a distinct and consequential idea.\footnote{Scheme itself is not
immutable: it has \code{set-car!} and \code{set-cdr!}, which change a pair in
place.  The original simply declines to use them, and \link{Section 3.3.1}---%
``Mutable List Structure''---is where they arrive.} Python leans the other way,
as most languages in common use do: its containers are mutable and its idioms
mutate freely, a list being sorted in place, a dictionary updated, an item
assigned.  Where we write in the Pythonic manner we shall therefore decline
those idioms and keep our data unchanging, as the original does.  Python helps
here more than one might expect, since it supplies both: the tuple is immutable
and the list is not, and choosing the tuple is often the whole of the
discipline.\footnote{Which default is right is one of the long-running
arguments in programming, and it is not settled.  Data that cannot change
cannot be changed behind your back, which rules out a whole family of bugs---%
two parts of a program holding what they each think is their own copy, and one
of them writing to it.  Against that, changing a structure in place avoids
building a new one, which can matter when the structures are large or the loop
is hot; though a language that expects immutability can recover much of the
cost by letting new structures share the parts of the old that did not change.
Languages take positions.  Some are immutable throughout and make you ask for
mutation explicitly; some, like Python, give you both and let the choice be a
matter of which type you reach for.  Rust, one of the more recent arrivals,
requires you to say which you mean at every binding: a name holds an unchanging
value unless you write \code{mut}.  That the question is still being answered
differently by each new language is a fair sign of how live it remains.}

The second is that Lisp builds everything from one thing.  The pair is its only
means of combination, and lists, trees, tables and expressions are all pairs
arranged differently.  Python has several primitive structures---tuple, list,
dictionary, set---each with a shape and a cost of its own.  We shall introduce
them as the chapter has use for them, and each time ask what the original was
getting from the pair that the new structure gives us for nothing.

Consider the task of designing a system to perform arithmetic with rational
numbers.  We could imagine an operation \code{add\_rat} that takes two rational
...
integers (a numerator and a denominator) and where \code{add\_rat} would be
...
\code{linear\_combination} should need to know about \code{a}, \code{b},
...
\code{linear\_combination}, it is irrelevant what \code{a}, \code{b}, \code{x},
...
as \code{linear\_combination} to pass its arguments along to \code{add} and
...
Python has an operator for exactly this.  Written \code{@}, it is the
multiplication of linear algebra: \code{a @ x + b @ y} is the linear
combination of two matrices, or two vectors, or two arrays of higher rank.  Nor
is it confined to the kinds of object the language happens to ship with---a new
kind can join in, and doing so turns out to require remarkably little.  What
that little is, and how it can possibly be enough, is the subject of
\link{Section 2.5}.
