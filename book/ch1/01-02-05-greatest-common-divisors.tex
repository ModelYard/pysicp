...
...         return a
...     else:
...         return TailCall(gcd, b, a % b)
\end{codeBlock}

\noindent
This generates an iterative process, whose number of steps grows as the
logarithm of the numbers involved.\footnote{Python supplies this one too, as
\code{math.gcd}.  It differs from the procedure above on negative arguments,
where it returns the positive divisor: \code{math.gcd(-206, 40)} is 2.  Our
version, as it happens, agrees---but only because Python's \code{\%} floors
rather than truncates, as we noted in \link{Section 1.2.4}.  Written with the
original's \code{remainder}, which truncates towards zero, the same procedure
would answer \( -2 \).  Two languages, the same algorithm, and a sign that
depends on a convention neither of them mentions.}
...
\begin{niceQuote}
\textbf{Lam\'e's Theorem:} If Euclid's Algorithm requires \( k \) steps to
...
\begin{exerciseGroup}
\phantomsection\label{Exercise 1.20}
\exerciseHeading{Exercise 1.20:} The process that a procedure
...
indicate the \code{\%} operations that are actually performed.  How many
\code{\%} operations are actually performed in the normal-order evaluation of
