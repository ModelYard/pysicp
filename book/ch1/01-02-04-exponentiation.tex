...
...         return product
...     else:
...         return TailCall(expt_iter, b, counter - 1, b * product)
\end{codeBlock}
...
the remainder operator \code{\%} by
...
\noindent
Note that the division in \code{fast\_expt} is written \code{//} and not
\code{/}.  As we saw in \link{Section 1.1.1}, \code{/} in Python always
produces a floating-point number, so \code{n / 2} would give \code{5.0} where
we want \code{5}, and the exponent would cease to be a whole number.
\code{//} is division that discards the remainder, and is what the original's
\code{/} does when its arguments are exact integers.\footnote{The remainder
operator likewise differs from the original's \code{remainder} on negative
numbers: Scheme's truncates towards zero, so that the remainder of \( -10 \)
and 3 is \( -1 \), while Python's floors, giving 2.  The difference does not
arise in \code{is\_even}, where the sign of the remainder does not matter, but
it will matter when we come to modular arithmetic in
\link{Section 1.2.6}.}

\noindent
\begin{revealTheSurface}
Python computes powers already.  The operator \code{**} raises one number to
another---\code{2 ** 10} is \code{1024}---and the built-in procedure
\code{pow} does the same, so nothing in this section is needed to work out an
exponent.  What is more, the algorithm we have just written is the one
\code{pow} uses; and \code{pow} takes an optional third argument, computing
\code{pow(b, n, m)} as \( b^n \) modulo \( m \) by exactly the method of
successive squaring set out above.  We will meet that computation again in
\link{Section 1.2.6}, where the original builds it by hand.

We build it here because the construction is the lesson.  The step from
\( \Theta(n) \) to \( \Theta(\log n) \) is not a fact about exponents but a
way of thinking---halve the problem rather than decrement it---and it recurs
throughout the book, in searching, in sorting, and in the arithmetic of very
large numbers.  \code{**} is that idea already had; what we want is to have it.
\end{revealTheSurface}

The process evolved by \code{fast\_expt} grows logarithmically with \( n \) in
both space and number of steps.  To see this, observe that computing
\( b^{2n} \) using \code{fast\_expt} requires only one more multiplication
...
example, \code{fast\_expt} for \( n \) = 1000 requires only 14
...
\begin{exerciseGroup}
\phantomsection\label{Exercise 1.16}
\exerciseHeading{Exercise 1.16:} Design a procedure that evolves
an iterative exponentiation process that uses successive squaring and uses a
logarithmic number of steps, as does \code{fast\_expt}.  (Hint: Using the
...
\phantomsection\label{Exercise 1.17}
\exerciseHeading{Exercise 1.17:} The exponentiation algorithms in
...
design a multiplication procedure analogous to \code{fast\_expt} that uses a
logarithmic number of steps.

\phantomsection\label{Exercise 1.18}
\exerciseHeading{Exercise 1.18:} Using the results of
...
\phantomsection\label{Exercise 1.19}
\exerciseHeading{Exercise 1.19:} There is a clever algorithm for
computing the Fibonacci numbers in a logarithmic number of steps.  Recall the
transformation of the state variables \( a \) and \( b \) in the \code{fib\_iter}
...
compute \( T^n \) using successive squaring, as in the \code{fast\_expt}
