...
it prints the sequence of approximations it generates, using \code{print} as
shown in \link{Exercise 1.22}.  Then find a
solution to \( x^x = 1000 \) by finding a fixed point of \( x \mapsto
\log(1000) / \log(x) \).  (Use \code{log} from Python's \code{math} module,
which computes natural logarithms: \code{from math import log}.)  Compare the number of steps
