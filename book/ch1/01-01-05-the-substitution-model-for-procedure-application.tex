...
To evaluate a compound expression whose operator names a compound procedure, the
interpreter follows much the same process as for the built-in operations we
described in \link{Section 1.1.3}.  That is, the interpreter evaluates the
subexpressions and applies the procedure named by the operator to the arguments
(which are the values of the operands).
...
\noindent
To illustrate this process, let's evaluate the call
...
\noindent
Thus the problem reduces to the evaluation of a call with two operands and an
operator \code{sum\_of\_squares}.  Evaluating this call involves three
subproblems.  We must evaluate the operator to get the procedure to be applied,
and we must evaluate the operands to get the arguments.  Now \code{5 + 1}
produces 6 and \code{5 * 2} produces 10, so we must apply the
\code{sum\_of\_squares} procedure to 6 and 10.  These values are substituted for
the formal parameters \code{x} and \code{y} in the body of
\code{sum\_of\_squares}, reducing the expression to
...
Python, like Lisp, uses applicative-order evaluation, partly because of the
additional efficiency obtained from avoiding multiple evaluations of expressions
such as those illustrated with \code{5 + 1} and \code{5 * 2} above and, more
...
\link{Section 4.2} we will modify the interpreter we have built to produce a
normal-order variant of the language.}  Python is not wholly without it: the
operators \code{and} and \code{or} leave their second operand unevaluated when
the first has settled the answer, which is why, as we remarked in
\link{Section 1.1.1}, they cannot be ordinary procedures.
