...
\begin{syntaxForm}
\meta{e1} if \meta{p1} else \meta{alternative}
\end{syntaxForm}
...
\begin{syntaxForm}
\meta{e1} if \meta{p1} else (
    \meta{e2} if \meta{p2} else (
        ...
        \meta{en} if \meta{pn} else \meta{en+1}))
\end{syntaxForm}
...
If none of the \( \langle{p}\rangle \)'s is found to be true---which in the nested
form above means the last \code{else} has been reached---the value is whatever
that final alternative supplies.  The original, whose \code{cond} may simply
run out of clauses, leaves the value undefined in that case; Python's
conditional expression has no such form, since every \code{if} in it must have
an \code{else}.
